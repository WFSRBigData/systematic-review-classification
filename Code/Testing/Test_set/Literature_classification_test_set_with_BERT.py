...
Code to test the final BERT model to classify papers in a specific topic as
"Relevant" or "Not relevant" based on their title and abstract. The used
literature data has been selected from academic databases based on keywords as
...
result_path = os.path.join(os.getcwd(), '../../..', 'Results', case_study, 'Test_set')
...
classifier = ktrain.load_predictor(os.path.join(model_path, model_name, "Final", "BERT_model"))
...
plt.savefig(os.path.join(result_path, "Test_ROC_{}.png".format(model_name)))
plt.show()

# Save the final results in a .txt file
results_file = open(os.path.join(result_path, "Test_results_{}.txt".format(model_name)), 
                    'w', encoding="utf8")
